...
exports.getUserMatchingPercentage = async (req, res, next) => {
  try {
    ...
    const matchingPercentage = await MatchingPercentage.findOne({
      userId: user.id,
    })
    .populate({
        path: 'matchingPercentage.scholarshipId',
        populate: [
          { path: 'fieldOfStudyId' },
          { path: 'courseTypeId' },
          { path: 'modeOfStudyId' },
    ...
    res
      .status(200)
      .send({ message: "Your Matching Percentage", matchingPercentage });
  } catch (error) {
