...
// CREATE a portfolio
 exports.createPortfolio=async (req, res,next) => {
  try {
    ...
    res.status(201).send({message:"protfolio created",portfolio:savedPortfolio});
  } catch (error) {
...
exports.updatePortfolio = async (req, res, next) => {
  ...
  try {
    ...
    if (portfolio.userID.id.toString() !== user._id.toString()) {
      return next(new CustomError("You are not authorized to update this portfolio", 403));
    }
    ...
    res.status(200).send({message:"your portfolio updated successfully",updatedPortfolio});
  } catch (error) {
    next(new CustomError(error.message, 500));
  }
};
...
exports.completePayment = async (req, res, next) => {
    ...
    const user = await User.findByIdAndUpdate(userID,{isBuyPortfolio:true,expBuyPortfolio:expDate},{ new: true });
...
exports.cancel=(req,res)=>{
    res.render(process.env.PROFILE_PAGE_URL)
}

exports.acceptProtfolio= async(req,res,next)=>{
    try {
        const portfolio = await Portfolio.findByIdAndUpdate(req.params.id,{isAccept:true},{ new: true }).populate("userID");
        if (!portfolio) {
            return next(new CustomError("Portfolio not found", 404));
        }
        const mailOptions = {
        ...
        res.status(200).send({ message: "Portfolio accepted successfully",portfolio });
...
exports.rejectPortfolio= async(req,res,next)=>{
    try {
        ...
        const portfolio = await Portfolio.findByIdAndUpdate(req.params.id,{isReject:true},{ new: true }).populate("userID");
        if (!portfolio) {
            return next(new CustomError("Portfolio not found", 404));
        }
        const mailOptions = {
        ...
        res.status(200).send({ message: "Portfolio rejected successfully" ,portfolio});
